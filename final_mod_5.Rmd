...
Using the number of arriving tourists as a rouge indicator, we can see that the number of arriving tourists has tripled from 558,000 in 2009 to 1,894,000 in 2019.
...
```{r}
tourist_info <- tibble(year=tourist_year, revenue=tourist_rev) %>% 
  mutate(revenue =  gsub(x = revenue,' m', 'e6', num))  #changing m into million
head(tourist_info)
```
